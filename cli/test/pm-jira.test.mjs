...
test("createHint elige la ayuda según el error de Jira, no una fija", () => {
  ...
  const epica = createHint(400, '{"errorMessages":["Por favor indicar épica para el caso de historias clasificadas como \\"Evolutiva\\""]}');
  assert.match(epica, /--parent/);
  assert.doesNotMatch(epica, /jira-fields\.json/);
  // (b) campo propio obligatorio → manda a jira-fields.json.
  const campo = createHint(400, '{"errors":{"customfield_10085":"Clasificación Historia es obligatorio."}}');
